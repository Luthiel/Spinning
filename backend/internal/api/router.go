...
func NewRouter(db *gorm.DB) *gin.Engine {
	...
	api := r.Group("/api")
	{
		...
		{
			health.GET("/summary", healthH.Summary)
			health.POST("/check/:skill_id", healthH.CheckSkill)
			health.POST("/check-all", healthH.CheckAll)
			health.GET("/reports/:skill_id", healthH.GetReport)
		}
	}
